	...
	while (letter != ESCAPE_KEY) {
		...
		if (letter == F_KEY_INIT || letter == SPEC_CHAR) {
			...
			if (letter == LEFT_ARROW) {
				...
				if (yCursor == MIN_ROW_INDEX || lines[yCursor - 1] == nullptr || curr->c == '\0')
					continue;
				...
				if (yCursor == MAX_ROW_INDEX || lines[yCursor + 1] == nullptr || curr->c == '\0')
					continue;
			...
			if (nodeManager.lineCount < 10)
				nodeManager.AddNode(&curr, &start, lines, xCursor, yCursor, '\n');
